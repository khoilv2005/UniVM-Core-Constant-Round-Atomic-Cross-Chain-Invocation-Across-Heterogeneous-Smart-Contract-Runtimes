...
async function findFirstFinalStatusBlock(
...
async function main() {
  if (network.name !== "bc1" && network.name !== "besu") {
    throw new Error(`GPACT 1b benchmark must run with the bc1 config, got network=${network.name}`);
  }
  ...
  if (![2, 3, 4, 5].includes(depth)) {
    throw new Error(`GPACT 1b depth must be one of 2,3,4,5; got ${depth}`);
  }
  ...
  for (let run = 1; run <= runs; run++) {
    const txId = ethers.keccak256(ethers.toUtf8Bytes(`gpact-1b-depth-${depth}-${run}-${Date.now()}`));
    ...
    while (Date.now() - startedAt < timeoutSeconds * 1000) {
      ...
      if (finalStatus === 5 || finalStatus === 6) {
        ...
        finalBlock = await findFirstFinalStatusBlock(controlAddress, txId, startBlock, latestBlock);
    ...
    if (finalStatus !== 5 && finalStatus !== 6) {
      throw new Error(`GPACT 1b txId=${txId} did not finish within ${timeoutSeconds}s`);
    }
    ...
    samples.push(sample);
    console.log(
      `[GPACT-1B][depth=${depth}][run=${run}] txId=${txId} status=${finalStatus} latency=${sample.latencySeconds.toFixed(3)}s blockLatency=${sample.blockLatencySeconds}s`,
    );
  }
  ...
  const outputPath = path.join(outputDir, `gpact-1b-d${depth}.json`);
