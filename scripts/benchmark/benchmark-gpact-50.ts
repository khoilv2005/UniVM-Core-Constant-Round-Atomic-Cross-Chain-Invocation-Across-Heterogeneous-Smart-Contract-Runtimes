...
async function main() {
  if (network.name !== "bc1" && network.name !== "besu") {
    throw new Error(`GPACT benchmark must run with the bc1 config, got network=${network.name}`);
  }
  ...
  for (let run = 1; run <= runs; run++) {
    const txId = ethers.keccak256(ethers.toUtf8Bytes(`gpact-benchmark-${run}-${Date.now()}`));
    ...
    if (finalStatus !== 5 && finalStatus !== 6) {
      throw new Error(`GPACT txId=${txId} did not finish within ${timeoutSeconds}s`);
    }
    ...
    samples.push(sample);
    console.log(
      `[GPACT][run=${run}] txId=${txId} status=${finalStatus} latency=${sample.latencySeconds.toFixed(3)}s blockLatency=${sample.blockLatencySeconds}s`,
    );
  }
  ...
  const result = {
    protocol: "gpact",
    mode: "live-e2e",
    workflow: "representative_train_hotel_2segment",
    segmentCount: 2,
    depthEquivalent: 3,
  ...
  const outputPath = path.join(outputDir, "gpact-50.json");
