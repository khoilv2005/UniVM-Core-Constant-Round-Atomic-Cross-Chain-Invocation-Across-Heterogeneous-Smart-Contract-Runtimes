...
async function main() {
  ...
  const lightClient = await hre.ethers.deployContract("LightClient", [3003n, 0n]);
  ...
  const adapter = await hre.ethers.deployContract("FabricStateProofAdapter", [
  ...
  const txId = 9601n;
  const lockedState = hre.ethers.toUtf8Bytes("fabric-locked-state");
  const meta = {
    chainId: hre.ethers.id("FABRIC:bc3"),
    contractId: hre.ethers.id("chaincode:HotelBooking"),
    schemaHash: hre.ethers.id("schema:HotelBooking:v1"),
    opId: hre.ethers.zeroPadValue(hre.ethers.toBeHex(txId), 32),
    lockEpoch: 1n,
    stateVersion: 1n,
  };
  const fields = {
    channelId: hre.ethers.id("channel:travel"),
    chaincodeNameHash: hre.ethers.id("chaincode:HotelBooking"),
    chaincodeVersionHash: hre.ethers.id("chaincode-version:v1"),
    endorsementPolicyHash: hre.ethers.id("policy:Org1MSP"),
    rwSetHash: hre.ethers.id("rwset:locked-state"),
    validationCodeHash: hre.ethers.id("validation:VALID"),
    txId: hre.ethers.id("fabric-tx:9601"),
  };
  ...
  await lightClient.submitBlockHeader(41n, hre.ethers.ZeroHash, leaf, 1n);
  await lightClient.finalizeBlock(41n);
  await bridge.connect(user).requestLockStates(txId, "travel", [await stateful.getAddress()], 2n, 3003n);
