/**
 * Deploy GPACT contracts to bc2 (hotel/flight chain).
 *
 * Run:
 *   npx hardhat --config hardhat.gpact-bc2.config.ts \
 *               --network besu run scripts/gpact/deploy-bc2.ts
 *
 * Deploys:
 *   GPACTEventSignerRegistry, GPACTCrosschainControl, GPACTHotel, GPACTFlight
 *
...
async function main() {
  ...
  await deployIfMissing(
    ...
    "gpactHotel",
    "GPACTHotel",
    [CONSTS.HOTEL_PRICE, CONSTS.HOTEL_REMAIN, control],
  );
  ...
  await deployIfMissing(
    ...
    "gpactFlight",
    "GPACTFlight",
    [CONSTS.FLIGHT_PRICE, CONSTS.FLIGHT_SEATS, control],
  );
