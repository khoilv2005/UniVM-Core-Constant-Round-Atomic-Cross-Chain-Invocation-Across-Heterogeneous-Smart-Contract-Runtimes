...
fn main() -> Result<()> {
    ...
    let elf_path = env::var("XSMART_RISC0_ELF")
        .map(PathBuf::from)
        .context("XSMART_RISC0_ELF must point to the compiled RISC Zero guest ELF")?;
    let image_id = env::var("XSMART_RISC0_IMAGE_ID")
        .context("XSMART_RISC0_IMAGE_ID must be the guest image ID bytes32")?;
    let out_path = env::var("XSMART_RISC0_PROOF_OUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("risc0-state-import-proof.json"));
    ...
    let prove_info = default_prover()
        ...
        .context("generate RISC Zero Groth16 receipt")?;
    ...
    let seal = receipt
        .inner
        .groth16()
        .context("expected Groth16 receipt for on-chain verification")?
        .seal
        .to_vec();
    let adapter_proof = (public_values.clone(), seal.clone()).abi_encode();
    ...
    let out = ProverOutput {
        ...
        journal_digest: hex0x(&journal_digest),
